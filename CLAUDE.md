...
2. **进度追踪权威 = 仓库内文档（2026-07-21 起，替代 GitHub Issues）**：账号迁移后旧 GitHub Issues 已失，改为**版本化的仓库内追踪**，三处协同：
   - `ROADMAP.md`：任务状态（⬜/🔄/✅）+ 决策日志 + 缺陷标注（**总账**）；
   - `docs/ISSUES.md`：开放任务/缺陷台账（带编号，如 `[4.1.2]`、`[BUG-73]`；一行一条，状态+归属+验收标准+备注）；
   - `docs/AIPaddle-项目进度管控.xlsx`：任务清单/仪表盘四表。
   - 动工前：在 `docs/ISSUES.md` 建/领条目；完成经用户验收 → 置 ✅ 并同步 `ROADMAP.md`；有遗留 → 条目保持 open，备注写清。**不再用 `gh issue` 命令追踪进度**（GitHub 仅承载代码与 PR）。
