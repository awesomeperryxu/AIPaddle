...
- 🔴 **`ALTER TABLE ADD COLUMN` 之后必须重建依赖视图**：视图哪怕写的是 `select m.*`，
  ...
  2026-08-07 实例（0040→0041）：给 `mcp_servers` 加 `credential_id` 后忘了
  ...
  查依赖：`pg_depend` + `pg_rewrite` 反查（见 0041 注释）；
...
## 🔴 外部系统对象同步铁律（2026-08-07 起，血泪教训）

从 WorkBuddy / Dify / 任何外部平台同步 Agent / Skill / Workflow / MCP / Plugin 时：

1. **坚决禁止占位数据落库**。不得写入 `endpoint` 为空的 MCP Server、无能力挂载的空壳 Agent、
   指向不存在对象的依赖声明。占位数据比缺失更糟——它让用户以为"配好了"，
   点下去才发现连不通，而排查要从头翻数据来源。
2. **级联关系必须完整创建**。同步一个对象前，先把它的下游依赖（一级、二级、多层级联）
   全部解析出来并创建完整、可执行。做不到就**整个不同步**，不留半成品。
3. **配置从真实来源获取**：优先读源平台的原始配置文件（如 WorkBuddy 的 `.mcp.json`），
   其次联网检索官方文档拿真实 endpoint。**严禁编造地址或密钥**——
   编造比占位更恶劣，占位至少诚实标着"待配置"。
4. **形态不兼容的不要硬塞**：客户端本地型 MCP（`command: node/npx` 启动进程）
   与服务端多租户平台架构不兼容，没有 endpoint 可填，**直接不导入**而不是造一个假地址。
5. **导入前先核对字段语义**。2026-08-07 实例：WorkBuddy 专家的 `plugin` 字段是
   **它自己的包名**（329/426 与 `agentName` 相同），导入脚本却当成"依赖的插件"写进
   `requiredPlugins`，造成 110 个 Agent 声明依赖一个既不存在、也不该存在的对象。
   用户看到"需要 invoice-verify-expert 但 MCP 页面没有"，其实那就是它自己的名字。
   **字段名相似不等于语义相同，导入映射必须逐字段确认来源含义。**
6. **清理错误数据的判定顺序**：先全面搜索补足级联对象 → 确实无法修复才删除；
   若属当时理解错误导致的错误数据（如第 5 条），直接删除该错误声明，
   但**不牵连删除承载真实业务价值的主体对象**。

> 同源教训：ADR-021 合并 MCP 到 Plugin 时没核对字段承接，导致连接层信息无处安放；
> 见 ADR-023 与其中的「否定性结论必须交叉验证」。
