...
export async function getAgentResources(_ctx: RequestContext, agentId: string): Promise<AgentResources> {
  if (!UUID_RE.test(agentId)) return { knowledgeBaseIds: [], skillIds: [], mcpServerIds: [] }
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('agent_resources')
    .select('resource_type,resource_id')
    .eq('agent_id', agentId)
    .in('resource_type', ['knowledge_base', 'skill', 'mcp_server'])
    .is('deleted_at', null)
...
export async function setAgentResources(
  ctx: RequestContext,
  agentId: string,
  next: { knowledgeBaseIds: string[]; skillIds: string[]; mcpServerIds: string[] },
  ...
  return { knowledgeBaseIds: kbIds, skillIds, mcpServerIds: mcpIds }
}
