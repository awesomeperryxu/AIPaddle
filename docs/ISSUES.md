...
| 4.1.6 | Agent Copilot 最简版（ADR-005） | B | 🔄 待验收 | `POST /api/agents/copilot`：描述→LLM 生成→zod Schema 校验(过则落 draft，不过 422)→`createAgent` 强制 draft(AI 不能发布)→`writeAudit`；agents-admin「AI 帮我建」弹窗；7 测试 |
