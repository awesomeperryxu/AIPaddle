...
| 1.4 | 确定 MVP 首个闭环场景 | 🔄 | 建议采纳审计意见：**Agent 管理**做第一条端到端闭环；需要你确认 |
...
| 2.3 | 权限模型：角色定义（参照 PRD 成员/租户模块），API 级校验方案 | 🔄 | **ADR-007 起草完成（2026-07-20，待 Perry 拍板）**：RBAC + 按操作(action)鉴权 + 默认拒绝 + 多角色取并集；4 角色×全模块权限矩阵（与 `ROLE_MATRIX` 契约 11 条一致）；执行=API 入口 `requirePermission()`（3.4），单一来源 `lib/auth/permissions.ts`；跨租户平台超管暂缓阶段 6。3 个判断点待拍板（Auditor 是否可 chat / 发布须审核 / 平台超管暂缓）。详见 `docs/adr/ADR-007-rbac-permission-model.md` |
...
| 2.5 | 统一 API 客户端与数据层设计（替换组件直接消费 mock 的现状） | 🔄 | **ADR-008 起草完成（2026-07-20，待 Perry 拍板）**：四层单向依赖（组件→API/Action→`lib/data/*`→Supabase 客户端）；组件禁直连 mock/supabase；`lib/data/*` 唯一碰库、首参 `ctx`、`server-only`；Repository 模式包 mock 逐页切（配合 3.5）；命名对齐现状 `lib/supabase/`。铁律已写入 CLAUDE.md。详见 `docs/adr/ADR-008-data-access-layer.md` |
...
| 2026-07-20 | 权限模型起草（ADR-007，冲刺 D1·D-2，**待拍板**）：RBAC+按操作(action)鉴权+默认拒绝+多角色并集；4 角色（Admin/Developer/User/Auditor）×全模块角色-权限矩阵（与 test-data `ROLE_MATRIX` 11 条断言一致）；执行=API 入口 `requirePermission()`（落地 3.4），单一来源 `lib/auth/permissions.ts`；跨租户 `tenant:manage` 平台超管暂缓阶段 6。3 待拍板点：Auditor 是否可 chat / 发布须走审核 / 平台超管暂缓 | ADR-007 |
| 2026-07-20 | 数据层设计起草（ADR-008，冲刺 D1·D-3，**待拍板**）：四层单向依赖（组件→API/Action→`lib/data/*`→Supabase 客户端）；组件禁直连 mock/supabase，`lib/data/*` 唯一碰库+首参 `ctx`+`server-only`；浏览器薄封装 `lib/api/client.ts` 只打 `/api/*`；Repository 模式包 mock 逐页切（配合 3.5，全切完删 mock-data.ts）；命名对齐现状 `lib/supabase/`（service 客户端=`lib/supabase/admin.ts`）；铁律写入 CLAUDE.md | ADR-008 |
