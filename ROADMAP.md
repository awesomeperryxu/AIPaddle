...
| 4.5.2 | 租户创建/编辑/停用 + 配额基础 | 🔄 | **本租户查看已完成（2026-07-21，D 道）**：`lib/data/tenant.ts` + `GET /api/tenant`（`tenant:read` Admin/Auditor）返回本租户信息+配额(token/存储/QPS)+用量(成员/Agent/知识库/token数)；实测 adminA 200、userA 403、RLS 只返本租户。**遵 ADR-007**：跨租户创建/停用/改他人配额=平台超管，留阶段 6；本租户编辑设置需 tenants 表 UPDATE RLS 策略（交 A 道 migration）后再做 |
